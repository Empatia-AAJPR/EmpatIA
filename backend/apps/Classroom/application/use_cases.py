...
from apps.Classroom.domain.exceptions import ClassroomNotFound, ConflictFieldsExceptions
...
class RegisterClassroomUseCase:
    def __init__(self, class_repo: IClassroomRepository, school_repo: ISchoolRepository) -> None:
        self.class_repo=class_repo
        self.school_repo=school_repo

    def execute(self, dto: ClassroomInDTO):
        if self.class_repo.exists_course(dto.course):
            raise ConflictFieldsExceptions(f'Course {dto.course} already exists')
...
class ListClassroomUseCase:
    ...
    def execute(self, school: UUID):
        ...
        return [
            ClassroomOutDTO.from_domain(classroom)
